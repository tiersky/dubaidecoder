...
const TIER_LABEL: Record<number, string> = { 10: 'Tier 1', 5: 'Tier 3', 2.5: 'Tier 2' };
...
export default function KPIGrid({ country }: KPIGridProps) {
  return (
    <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
      ...
      <MetricCard
        label="Spends per Trip"
        value={country.spendsPerTrip.toFixed(1)}
      ...
      <MetricCard
        label="Tourist Departures / Pop."
        value={`${country.touristDeparturesRatio.toFixed(2)}×`}
      ...
      <MetricCard
        label="Search Vol. – Flight to AlUla"
        value={country.searchVolume.toLocaleString()}
        icon={icon('M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607z')}
      />

      <MetricCard
        label="GDP Growth"
        value={`${(country.gdpGrowth * 100).toFixed(1)}%`}
        icon={icon('M2.25 18L9 11.25l4.306 4.307a11.95 11.95 0 015.814-5.519l2.74-1.22m0 0l-5.94-2.28m5.94 2.28l-2.28 5.941')}
      />

      <MetricCard
        label="Disposable Income"
        value={`$${country.disposableIncome.toLocaleString()}`}
      ...
      <MetricCard
        label="Media Cost"
        value={country.mediaCost.toFixed(1)}
      ...
      <MetricCard
        ...
        value={`${country.globalExplorers.toFixed(1)}M`}
        icon={icon('M12 21a9.004 9.004 0 008.716-6.747M12 21a9.004 9.004 0 01-8.716-6.747M12 21c2.485 0 4.5-4.03 4.5-9S14.485 3 12 3m0 18c-2.485 0-4.5-4.03-4.5-9S9.515 3 12 3m0 0a8.997 8.997 0 017.843 4.582M12 3a8.997 8.997 0 00-7.843 4.582m15.686 0A11.953 11.953 0 0112 10.5c-2.998 0-5.74-1.1-7.843-2.918m15.686 0A8.959 8.959 0 0121 12c0 .778-.099 1.533-.284 2.253m0 0A17.919 17.919 0 0112 16.5c-3.162 0-6.133-.815-8.716-2.247m0 0A9.015 9.015 0 013 12c0-1.605.42-3.113 1.157-4.418')}
      />

      <MetricCard
        label="Budget Split (index)"
        value={country.budgetSplitMetric.toFixed(2)}
      ...
      <MetricCard
        label="Market Tiering"
        value={TIER_LABEL[country.marketTiering] ?? country.marketTiering.toFixed(1)}
      ...
      <MetricCard
        label="Audience Ratio vs. Pop."
        value={`${(country.audienceRatio * 100).toFixed(2)}%`}
