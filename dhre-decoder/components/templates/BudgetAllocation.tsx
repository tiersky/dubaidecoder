...
export default function BudgetAllocation({ allCountries }: BudgetAllocationProps) {
  const [xAxis, setXAxis] = useState('priceToIncome');
  const [yAxis, setYAxis] = useState('grossRentalYield');
  const [bubbleSize, setBubbleSize] = useState('weightedScore');
  ...
  const allocations = useMemo(
    () => computeAllocations(allCountries, modelWeights, totalBudget, excludedCodes),
    [allCountries, modelWeights, totalBudget, excludedCodes]
  );
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-400 mt-1">
          Interactive market analysis and investment budget optimization
        </p>
      </div>
      ...
      <GlassCard>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          <SelectDropdown label="X-Axis" options={axisOptions} value={xAxis} onChange={setXAxis} />
          <SelectDropdown label="Y-Axis" options={axisOptions} value={yAxis} onChange={setYAxis} />
          <SelectDropdown label="Bubble Size" options={bubbleSizeOptions} value={bubbleSize} onChange={setBubbleSize} />
        </div>
        <BubbleChart allCountries={allCountries} xAxis={xAxis} yAxis={yAxis} bubbleSize={bubbleSize} />
      </GlassCard>

      {/* Budget Calculator + Model Weights + Pie Chart */}
        ...
        <div className="space-y-4">
          <GlassCard>
            ...
            <p className="text-xs text-slate-400 mb-4">
              Based on weighted score analysis — toggle countries on/off below
            </p>
