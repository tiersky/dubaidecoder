...
import { INDEX_KEYS, INDEX_LABELS, INDEX_HIGHER_BETTER, ModelWeights } from '@/types';
  ...
  return (
    <GlassCard>
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className="text-sm font-semibold text-slate-800">Model Weights</h3>
          ...
          {INDEX_KEYS.map((key) => (
            <div key={key} className="space-y-1">
              ...
              <p className="text-[10px] text-slate-400">
                {INDEX_HIGHER_BETTER[key] ? 'Higher = bigger target' : 'Lower = UAE more attractive'}
              </p>
              <input
                type="number"
                min={0}
