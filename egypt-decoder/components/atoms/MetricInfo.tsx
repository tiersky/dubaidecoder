...
import { INDEX_LABELS, INDEX_SOURCES, INDEX_DESCRIPTIONS } from "@/types";
 ...
 */
export default function MetricInfo({ metricKey, label }: MetricInfoProps) {
  const title = label ?? INDEX_LABELS[metricKey] ?? metricKey;
  const description = INDEX_DESCRIPTIONS[metricKey];
  const source = INDEX_SOURCES[metricKey];
