import MetricCard from '@/components/molecules/MetricCard';
import { Country } from '@/types';
import { formatPopulation } from '@/lib/utils';

interface KPIGridProps {
  country: Country;
}

const icon = (d: string) => (
  <svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d={d} />
  </svg>
);

const TIER_LABEL: Record<number, string> = {
  10: 'Tier 1',
  6: 'Tier 2',
  3: 'Tier 3',
  1: 'Tier 4',
};

export default function KPIGrid({ country }: KPIGridProps) {
  return (
    <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
      <MetricCard
        label="Population"
        metricKey="population"
        value={formatPopulation(country.population)}
        icon={icon('M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-3.07M12 6.375a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zm8.25 2.25a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0z')}
      />

      <MetricCard
        label="2025 Visitors"
        metricKey="visitors2025"
        value={country.visitors2025 > 0 ? formatPopulation(country.visitors2025) : 'Strategic add'}
        icon={icon('M15 10.5a3 3 0 11-6 0 3 3 0 016 0z M19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1115 0z')}
      />

      <MetricCard
        label="YoY Visitor Growth"
        metricKey="yoyGrowth"
        value={`${(country.yoyGrowth * 100).toFixed(0)}%`}
        icon={icon('M2.25 18L9 11.25l4.306 4.307a11.95 11.95 0 015.814-5.519l2.74-1.22m0 0l-5.94-2.28m5.94 2.28l-2.28 5.941')}
      />

      <MetricCard
        label="Audience Ratio / Pop."
        metricKey="audienceRatio"
        value={country.audienceRatio.toLocaleString()}
        icon={icon('M18 18.72a9.094 9.094 0 003.741-.479 3 3 0 00-4.682-2.72m.94 3.198l.001.031c0 .225-.012.447-.037.666A11.944 11.944 0 0112 21c-2.17 0-4.207-.576-5.963-1.584A6.062 6.062 0 016 18.719m12 0a5.971 5.971 0 00-.941-3.197m0 0A5.995 5.995 0 0012 12.75a5.995 5.995 0 00-5.058 2.772m0 0a3 3 0 00-4.681 2.72 8.986 8.986 0 003.74.477m.94-3.197a5.971 5.971 0 00-.94 3.197M15 6.75a3 3 0 11-6 0 3 3 0 016 0zm6 3a2.25 2.25 0 11-4.5 0 2.25 2.25 0 014.5 0zm-13.5 0a2.25 2.25 0 11-4.5 0 2.25 2.25 0 014.5 0z')}
      />

      <MetricCard
        label="Tourist Departures"
        metricKey="touristDepartures"
        value={formatPopulation(country.touristDepartures)}
        icon={icon('M9 6.75V15m6-6v8.25m.503 3.498l4.875-2.437c.381-.19.622-.58.622-1.006V4.82c0-.836-.88-1.38-1.628-1.006l-3.869 1.934c-.317.159-.69.159-1.006 0L9.503 3.252a1.125 1.125 0 00-1.006 0L3.622 5.689C3.24 5.88 3 6.27 3 6.695V19.18c0 .836.88 1.38 1.628 1.006l3.869-1.934c.317-.159.69-.159 1.006 0l4.994 2.497c.317.158.69.158 1.006 0z')}
      />

      <MetricCard
        label="Departures / 2025 Visitors"
        metricKey="departuresToVisitors"
        value={`${(country.departuresToVisitors * 100).toFixed(1)}%`}
        icon={icon('M7.5 21L3 16.5m0 0L7.5 12M3 16.5h13.5m0-13.5L21 7.5m0 0L16.5 12M21 7.5H7.5')}
      />

      <MetricCard
        label="Outbound Trip Spends"
        metricKey="outboundSpends"
        value={`$${country.outboundSpends.toLocaleString()}B`}
        icon={icon('M2.25 18.75a60.07 60.07 0 0115.797 2.101c.727.198 1.453-.342 1.453-1.096V18.75M3.75 4.5v.75A.75.75 0 013 6h-.75m0 0v-.375c0-.621.504-1.125 1.125-1.125H20.25M2.25 6v9m18-10.5v.75c0 .414.336.75.75.75h.75m-1.5-1.5h.375c.621 0 1.125.504 1.125 1.125v9.75c0 .621-.504 1.125-1.125 1.125h-.375m1.5-1.5H21a.75.75 0 00-.75.75v.75m0 0H3.75m0 0h-.375a1.125 1.125 0 01-1.125-1.125V15m1.5 1.5v-.75A.75.75 0 003 15h-.75M15 10.5a3 3 0 11-6 0 3 3 0 016 0zm3 0h.008v.008H18V10.5zm-12 0h.008v.008H6V10.5z')}
      />

      <MetricCard
        label="Media Cost CPM"
        metricKey="mediaCost"
        value={country.mediaCost !== null ? `$${country.mediaCost.toFixed(2)}` : 'N/A'}
        icon={icon('M2.25 8.25h19.5M2.25 9h19.5m-16.5 5.25h6m-6 2.25h3m-3.75 3h15a2.25 2.25 0 002.25-2.25V6.75A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25v10.5A2.25 2.25 0 004.5 19.5z')}
      />

      <MetricCard
        label="GDP per Capita"
        metricKey="gdpPerCapita"
        value={`$${country.gdpPerCapita.toLocaleString()}`}
        icon={icon('M12 6v12m-3-2.818l.879.659c1.171.879 3.07.879 4.242 0 1.172-.879 1.172-2.303 0-3.182C13.536 12.219 12.768 12 12 12c-.725 0-1.45-.22-2.003-.659-1.106-.879-1.106-2.303 0-3.182s2.9-.879 4.006 0l.415.33M21 12a9 9 0 11-18 0 9 9 0 0118 0z')}
      />

      <MetricCard
        label="Daily Flight Seats"
        metricKey="flightSeats"
        value={country.flightSeats.toLocaleString()}
        icon={icon('M6 12L3.269 3.126A59.768 59.768 0 0121.485 12 59.77 59.77 0 013.27 20.876L5.999 12zm0 0h7.5')}
      />

      <MetricCard
        label="Market Tier"
        metricKey="marketTier"
        value={TIER_LABEL[country.marketTier] ?? country.marketTier.toFixed(0)}
        icon={icon('M3.75 21h16.5M4.5 3h15M5.25 3v18m13.5-18v18M9 6.75h1.5m-1.5 3h1.5m-1.5 3h1.5m3-6H15m-1.5 3H15m-1.5 3H15M9 21v-3.375c0-.621.504-1.125 1.125-1.125h3.75c.621 0 1.125.504 1.125 1.125V21')}
      />
    </div>
  );
}
